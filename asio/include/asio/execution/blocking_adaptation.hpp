...
struct blocking_adaptation_t
{
  ...
  {
    ASIO_CONSTEXPR convertible_from_blocking_adaptation_t(blocking_adaptation_t)
    {
    }
  };
  ...
  friend ASIO_CONSTEXPR blocking_adaptation_t query(
    ...
    ASIO_NOEXCEPT_IF((
      is_nothrow_query<const Executor&, blocking_adaptation_t<>::disallowed_t>::value))
...
#if defined(_MSC_VER) // Visual C++ wants the type to be qualified.
    ASIO_NOEXCEPT_IF((
      is_nothrow_query<const Executor&, blocking_adaptation_t<>::allowed_t>::value))
#elif !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((
